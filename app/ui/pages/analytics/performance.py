"""Performance tab of the Analytics & Risk page.

The tab calls ``analytics.drawdown_series`` directly to derive the area-chart
values from ``PerformanceView.portfolio_navs_raw``. That is the explicit A1
exception to the usual page -> service -> domain flow: max drawdown is a service
fact, while the full drawdown series only feeds one visual.
"""
