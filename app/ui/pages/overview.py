...
def render() -> None:
    ...
    thesis_pill_html = "".join([
        f'<span class="badge {"badge-green" if _PLACEHOLDER_THESIS_STATUS.get(t, "intact") == "intact" else "badge-amber" if _PLACEHOLDER_THESIS_STATUS.get(t, "intact") == "watch" else "badge-red"}" style="margin-right: 2px;">{t}</span>'  # noqa: E501
    ...
    st.markdown(f"""
        ...
        <div style="width: 100%; height: 4px; background: var(--surface2); border-radius: 2px; margin-bottom: 24px; overflow: hidden;">  # noqa: E501
            <div style="width: 0%; height: 100%; background: var(--green);"></div>
        </div>
    """, unsafe_allow_html=True)
    
    sorted_positions = sorted(
        live_positions.values(),
    ...
    for p in sorted_positions:
        ...
        val = "—" if is_stale or p.live_value_eur is None else format_eur(p.live_value_eur, signed=False).replace("€", "")  # noqa: E501
        gain = "—" if is_stale or p.unrealised_gain_eur is None else format_eur(p.unrealised_gain_eur, signed=True).replace("€", "")  # noqa: E501
        ...
        gain_class = "gain-neutral" if is_stale else ("gain-positive" if unrealised_gain_eur_amount > 0 else "gain-negative" if unrealised_gain_eur_amount < 0 else "gain-neutral")  # noqa: E501
        ...
        weight_html = f"""
            <div style="display: flex; align-items: center; gap: 4px;">
                <span>{weight_pct:.1f}%</span>
                <div style="width: 30px; height: 4px; background: var(--surface2); border-radius: 2px; overflow: hidden;">  # noqa: E501
                    <div style="width: {weight_pct}%; height: 100%; background: var(--blue);"></div>
                </div>
            </div>
        """
        ...
        tbody.append(f"""
            <tr class="{row_class}">
                ...
                <td style="color: var(--text3);">{ccy}</td>
                <td class="font-mono text-right">{price}</td>
                <td class="font-mono text-right">{shares}</td>
                <td class="font-mono text-right">{cost}</td>
                <td class="font-mono text-right"><strong>{val}</strong></td>
                <td class="font-mono text-right {gain_class}">{gain}</td>
                <td class="font-mono">{weight_html}</td>
                <td class="text-center">{horizon}</td>
                <td class="text-center">{thesis}</td>
                <td class="font-mono text-center" style="color: var(--text3);">{lots}</td>
            </tr>
        """)
        
    table_html = f"""
        <table class="positions-table" style="width: 100%; border-collapse: collapse; text-align: left; font-size: 13px;">  # noqa: E501
            <thead>
                <tr style="border-bottom: 1px solid var(--border); color: var(--text3); text-transform: uppercase; font-size: 10px; letter-spacing: 0.05em;">  # noqa: E501
    ...
    st.markdown(f'<div class="metric-card" style="padding: 0; overflow-x: auto;">{table_html}</div>', unsafe_allow_html=True)  # noqa: E501
    ...
    st.markdown(f"""
        <div style="margin-top: 16px; font-size: 11px; font-family: 'DM Mono', monospace; color: var(--text3); text-align: right;">  # noqa: E501
            {status_text}
        </div>
    """, unsafe_allow_html=True)