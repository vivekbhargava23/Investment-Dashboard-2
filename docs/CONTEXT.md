# CONTEXT — auto-generated 2026-06-02 22:02:31 UTC
...
│   │   ├── parser.py
│   │   └── planner.py
│   ├── ticker_resolver_composite
│   │   ├── __init__.py
│   │   └── adapter.py
│   ├── ticker_resolver_finnhub
│   │   ├── __init__.py
│   │   └── adapter.py
│   ├── yfinance_feed
│   │   └── __init__.py
...
│   ├── TICKET-CSV-13-zero-touch-import.md
...
│   ├── TICKET-M10-worktree-workflow-polish.md
...
class YfinanceLiveFxAdapter:
    def __init__(self, current_ttl_seconds: int) -> None
...
class CompositeTickerResolver:
    def __init__(self, primary: TickerResolver, fallbacks: list[TickerResolver]) -> None
    def resolve(self, query: str, limit: int) -> list[TickerMatch]
    def lookup(self, symbol: str) -> TickerMatch | None
    def clear_cache(self) -> None
```
...
```python
def build_ticker_resolver(cache_path: Path, finnhub_api_key: str | None) -> TickerResolver
```
...
### app/adapters/yfinance_ohlc/adapter.py
...
### app/adapters/yfinance_price/adapter.py
...
### app/adapters/yfinance_resolver/adapter.py
...
```python
def main(input_path: Path, output_path: Path | None, dry_run: bool, force: bool, price_provider: YfinancePriceAdapter | None, interactive: bool | None) -> int
```
...
def build_correlation_view(*, repo: TransactionRepository, price_feed: PriceProvider, fx_feed: LiveFxProvider, ohlc: OhlcDataProvider, as_of: date, window_days: int) -> CorrelationView
```
...
def build_technicals_view(*, ticker: str, period: str, repo: TransactionRepository, price_feed: PriceProvider, ohlc: OhlcDataProvider, as_of: date, freq: AggregationFreq | None) -> TechnicalsView
```
...
### app/services/isin_autoresolve.py
...
class AutoResolveResult:
    ...
    reason: str
def autoresolve_isin(isin: str, description_hint: str, *, resolver: TickerResolver, company_provider: CompanyDataProvider) -> AutoResolveResult
```
...
```python
def get_ohlc_history(ticker: str, period: ChartPeriod, *, provider: OhlcDataProvider, freq: AggregationFreq | None) -> OhlcSeries
...
```python
def build_transaction(ticker: str, tx_type: TransactionType, trade_date: date, shares: Decimal, eur_total: Decimal, fees_eur: Decimal, currency: Currency, price_provider: PriceProvider, fx_provider: HistoricalFxProvider) -> tuple[Transaction, Decimal | None]
```

### app/services/valuation.py

```python
def compute_live_positions(transactions: Sequence[Transaction], price_provider: PriceProvider, fx_provider: LiveFxProvider) -> dict[str, LivePosition]
def get_live_positions_cached(*, repo: TransactionRepository, price_provider: PriceProvider, fx_provider: LiveFxProvider, ttl_seconds: float) -> dict[str, LivePosition]
...
def clear_caches(price_provider: PriceProvider, fx_provider: LiveFxProvider) -> None
```
...
### app/ui/components/isin_mapper.py

```python
def suggest_kind(ticker: str) -> InstrumentKind | None
def render_kind_selector(key: str, *, suggested: InstrumentKind | None) -> InstrumentKind | None
def render_isin_mapper_row(isin: str, description: str, *, key_prefix: str) -> tuple[TickerMatch | None, InstrumentKind | None]
def build_mapping(isin: str, ticker: str, kind: InstrumentKind, description: str, current_doc: IsinMapDocument) -> IsinMapDocument
```
...
- `app/ui/pages/overview.py` — get_isin_map_repo,
