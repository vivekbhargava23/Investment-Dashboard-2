...
None-return neutral/"n/a" handling, and stale-position exclusion.
"""
...
ReturnsMap = dict[str, dict[ReturnWindow, Decimal | None]]


def _live(ticker: str, value_eur: str) -> LivePosition:
    """A non-stale LivePosition with an exact live EUR value."""
...
def _returns(**rows: Decimal | None) -> ReturnsMap:
    """Build a returns map where each kwarg sets the 30D return for that ticker."""
    return {ticker: {ReturnWindow.D30: pct} for ticker, pct in rows.items()}
...
def test_values_match_live_value_eur() -> None:
    ...
    returns = _returns(AAA=Decimal("1"), BBB=Decimal("2"), CCC=Decimal("3"))
    fig = build_treemap_figure(
        positions, returns, ReturnWindow.D30, name_lookup={}
    )
...
def test_clamp_is_symmetric_and_colors_carry_raw_returns() -> None:
    ...
    returns = _returns(UP=Decimal("30"), DOWN=Decimal("-30"))
    fig = build_treemap_figure(
        positions, returns, ReturnWindow.D30, name_lookup={}
    )
    assert fig is not None
...
def test_none_return_is_neutral_and_hover_says_na() -> None:
    ...
    returns: ReturnsMap = {
        ...
        "BBB": {ReturnWindow.D30: Decimal("5")},
    }
    fig = build_treemap_figure(
        positions, returns, ReturnWindow.D30, name_lookup={"AAA": "Alpha"}
    )
    assert fig is not None
    trace = fig.data[0]
...
def test_stale_position_absent_from_figure() -> None:
    ...
    returns = _returns(LIVE=Decimal("2"))
    fig = build_treemap_figure(
        positions, returns, ReturnWindow.D30, name_lookup={}
    )
    assert fig is not None
...
def test_render_treemap_draws_figure() -> None:
    ...
    returns = _returns(AAA=Decimal("1"))
    with patch("app.ui.components.treemap.st") as mock_st:
        render_treemap(positions, returns, ReturnWindow.D30, name_lookup={})
...
def test_hover_weight_is_share_of_total_value() -> None:
    positions = {"AAA": _live("AAA", "750"), "BBB": _live("BBB", "250")}
    returns = _returns(AAA=Decimal("1"), BBB=Decimal("1"))
    fig = build_treemap_figure(
        positions, returns, ReturnWindow.D30, name_lookup={}
    )
    assert fig is not None
